...
function App() {
  ...
  const handleApiError = useCallback((error: any) => {
    ...
    if (error.status === 429 || (error.message && (error.message.includes('429') || error.message.includes('quota')))) {
        ...
        addNewsItem({
            ...
            text: "AI 通信繁忙 (429)。正在冷却...",
            type: 'neutral'
        });
    } else {
        console.error("Non-critical API Error:", error);
    }
  }, [addNewsItem]);

  const fetchNewGoal = useCallback(async () => {
    if (isGeneratingGoal || !aiEnabledRef.current) return;
    if (Date.now() < apiBackoffRef.current) return;
  ...
  }, [isGeneratingGoal, handleApiError]); 

  const fetchNews = useCallback(async () => {
    if (Date.now() < apiBackoffRef.current) return;

    // Reduced chance to fetch news per tick to avoid rate limits (approx every 40s)
  ...
  useEffect(() => {
    ...
    addNewsItem({ id: Date.now().toString(), text: "欢迎来到天空都市。地形生成完毕。", type: 'positive' });
  ...
  const handleTileClick = useCallback((x: number, y: number) => {
    ...
    if (tool === BuildingType.None) {
      if (currentTile.buildingType !== BuildingType.None) {
        ...
        if (currentStats.money >= demolishCost) {
            ...
            addNewsItem({id: Date.now().toString(), text: "无法支付拆除费用。", type: 'negative'});
        }
      }
      return;
    }
    ...
    if (currentTile.buildingType === BuildingType.None) {
      if (currentStats.money >= buildingConfig.cost) {
        ...
        addNewsItem({id: Date.now().toString() + Math.random(), text: `国库资金不足以建造${buildingConfig.name}。`, type: 'negative'});
      }
    }
  ...
  const handleClaimReward = () => {
    if (currentGoal && currentGoal.completed) {
      ...
      addNewsItem({id: Date.now().toString(), text: `目标达成！${currentGoal.reward} 已存入国库。`, type: 'positive'});
      setCurrentGoal(null);
      fetchNewGoal();
    }
  };
