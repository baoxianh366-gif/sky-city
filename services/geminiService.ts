...
const goalSchema = {
  ...
  properties: {
    description: {
      type: Type.STRING,
      description: "A short, creative description of the goal from the perspective of city council or citizens (in Chinese).",
    },
    targetType: {
      type: Type.STRING,
...
export const generateCityGoal = async (stats: CityStats, grid: Grid): Promise<AIGoal | null> => {
  ...
  const prompt = `You are the AI City Advisor for a simulation game. Based on the current city stats, generate a challenging but achievable short-term goal for the player to help the city grow. Return JSON. The 'description' MUST be in Simplified Chinese.`;
...
const newsSchema = {
  type: Type.OBJECT,
  properties: {
    text: { type: Type.STRING, description: "A one-sentence news headline representing life in the city (in Chinese)." },
...
export const generateNewsEvent = async (stats: CityStats, recentAction: string | null): Promise<NewsItem | null> => {
  ...
  const prompt = "Generate a very short, isometric-sim-city style news headline based on the city state. Can be funny, cynical, or celebratory. The text MUST be in Simplified Chinese.";
